...
void ArvoreABB::imprimeIntervalo(int x, int y){
    auxImprimeIntervalo(raiz, x, y);
    cout << endl;
}
void ArvoreABB::auxImprimeIntervalo(No *p, int x, int y){
    if(p != NULL){
        if(p->getInfo() > x){
...
void ArvoreABB::imprimeDecrescente(){
    auxImprimeDecrescente(raiz);
    cout << endl;
}
void ArvoreABB::auxImprimeDecrescente(No *p){
    if(p != NULL){
...
void ArvoreABB::insereDoVet(int n, int *Vet){
    int cont = 0;
    raiz = auxInsereDoVet(raiz, n, &cont, Vet);
}
No* ArvoreABB::auxInsereDoVet(No *p, int n, int *c, int *vet){
    if((*c) < n) {
        if (p == NULL) {
            p = new No();
            p->setInfo(vet[*c]);
            ...
            if (vet[*c] < p->getInfo()) {
                p->setEsq(auxInsereDoVet(p->getEsq(), n, c, vet));
            } else {
                p->setDir(auxInsereDoVet(p->getDir(), n, c, vet));
            }
...
int* ArvoreABB::insereVetCrescente(){
    int n = auxGetTamanho(raiz);

    int *v = new int[n];
    int cont = 0;

    auxInsereVetCrescente(raiz, v, &cont, n);

    return v;
}
int ArvoreABB::getTamanho(){
    return auxGetTamanho(raiz);
}
int ArvoreABB::auxGetTamanho(No *p){
    if(p == NULL){
        return 0;
    }
    return 1 + auxGetTamanho(p->getEsq()) + auxGetTamanho(p->getDir());
}
void ArvoreABB::auxInsereVetCrescente(No *p, int *v, int *cont, int n){
    if(p != NULL && (*cont) < n){
        auxInsereVetCrescente(p->getEsq(), v, cont, n);
        v[*cont] = p->getInfo();
        (*cont)++;
        auxInsereVetCrescente(p->getDir(), v, cont, n);
    }
}
