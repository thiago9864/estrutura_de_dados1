...
int Arvore::auxImpares(No *p)
{
    if(p==NULL)
    {
        return 0;
    }
    int cont = auxImpares(p->getEsq()) + auxImpares(p->getDir());
    if(p->getInfo() % 2 != 0)
    {
        cont++;
    }
...
int Arvore::maiores(int x)
{
    return auxMaiores(raiz, x);
}
int Arvore::auxMaiores(No* p, int x)
{
    if(p==NULL)
    {
        return 0;
    }
    int m = auxMaiores(p->getEsq(), x) + auxMaiores(p->getDir(), x);
...
float Arvore::mediaNivel(int nivel)
{
    int cont = 0;
    return auxMediaNivel(raiz, nivel, 0, &cont) / (float)cont;
}
float Arvore::auxMediaNivel(No *p, int k, int n, int *cont)
{
    if(p == NULL)
    {
        return 0;
    ...
    if(n < k){
        soma = auxMediaNivel(p->getEsq(), k, n+1, cont) + auxMediaNivel(p->getDir(), k, n+1, cont);
    }
